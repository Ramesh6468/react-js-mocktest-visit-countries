...
class VisitCountries extends Component {
  state = {nationList: initialCountriesList}

  removeNationItem = id => {
    const {nationList} = this.state
    const removedCountry = nationList.map(each => {
      if (each.id === id) {
        const removedItem = {
          ...each,
    ...
    this.setState({nationList: removedCountry})
  }

  onClickVisitButton = id => {
    const {nationList} = this.state
    const visitCountry = nationList.map(each => {
      if (each.id === id) {
    ...
    this.setState({nationList: visitCountry})
  }

  render() {
    const {nationList} = this.state
    console.log(nationList)
    const updatedList = nationList.filter(each => each.isVisited === true)
    return (
      <div className="bgContainer">
        ...
        <div className="nationCard">
          {updatedList.length === 0 ? (
            ...
            <ul className="nationlist">
              {updatedList.map(each => (
                <NationItem
                  key={each.id}
                  nationDetails={each}
                  removeNationItem={this.removeNationItem}
                />
              ))}
            </ul>
          )}
